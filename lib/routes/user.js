...
const addMovieToFavorites = {
    ...
    options: {
        ...
        validate: {
            params: Joi.object({
                id: Joi.number().integer().required().description('User id'),
                movieId: Joi.number().integer().required().description('Movie id')
            })
        }
    },
    handler: async (request, h) => {
        ...
        return await userService.addMovieToFavorites(request.params.id, request.params.movieId);
    }
};
